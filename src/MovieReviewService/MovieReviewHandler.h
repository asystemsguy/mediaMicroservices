...
void MovieReviewHandler::UploadMovieReview(int64_t req_id, const std::string& movie_id, int64_t review_id, int64_t timestamp, const std::map<std::string, std::string> & carrier) {
  ...
  auto collection = mongoc_client_get_collection(mongodb_client, "movie-review", "movie-review");
  ...
  if (!found) {
    ...
    started_mongo_find = std::chrono::high_resolution_clock::now(); 
    bool plotupdate = mongoc_collection_find_and_modify(collection, query, nullptr, update, nullptr, false, false, true, &reply, &error);
    timetaken_mongo_call = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now()-started_mongo_find).count();
    timetaken_by_external_calls = timetaken_by_external_calls + timetaken_mongo_call;
...
void MovieReviewHandler::ReadMovieReviews(
  ...
  double timetaken = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now()-startt).count();
  LOG(warning) << timetaken-timetaken_by_external_calls; 
  span->Log({{"runtime_ms",timetaken-timetaken_by_external_calls}});
  span->Finish();
  LOG(warning) << "end";
